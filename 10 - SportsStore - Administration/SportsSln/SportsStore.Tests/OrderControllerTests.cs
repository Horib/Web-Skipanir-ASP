...
namespace SportsStore.Tests
{
    public class OrderControllerTests
    {
        ...
        {
            // Arramge - create a mock repository
        ...
        public void Cannot_Checkout_Invalid_ShippingDetails()
        {
            ...
            Mock<IOrderRepository> mock = new Mock<IOrderRepository>();
            // Arrange- creat a cart with one item
            Cart cart = new Cart();
            cart.AddItem(new Product(), 1);
            // Arrange - creat an instanc of the controller
            OrderController target = new OrderController(mock.Object, cart);
            ...
            mock.Verify(m => m.SaveOrder(It.IsAny<Order>()), Times.Never);
            // Assert - check that the method is returing the default view
            Assert.True(string.IsNullOrEmpty(result?.ViewName));
            // Assert - check that I am passing an invalid model to the view
            Assert.False(result?.ViewData.ModelState.IsValid);
        }
        ...
        public void Can_Checkout_And_Submit_Order()
        {
            ...
            Cart cart = new Cart();
            cart.AddItem(new Product(), 1);
            // Arrange - creater an instance of the controller
            OrderController target = new OrderController(mock.Object, cart);
